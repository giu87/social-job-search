...
public class AlchemyAPI_KeywordParams extends AlchemyAPI_Params {
   ...
   public void setKeywordExtractMode(String keywordExtractMode) {
      if (!keywordExtractMode.equals(AlchemyAPI_KeywordParams.EXTRACT_MODE_STRICT)) {
         throw new RuntimeException("Invalid setting " + keywordExtractMode
               + " for parameter keywordExtractMode");
      }
   ...
   public void setSourceText(String sourceText) {
      if (!sourceText.equals(AlchemyAPI_KeywordParams.CLEANED)
            && !sourceText.equals(AlchemyAPI_KeywordParams.CLEANED_OR_RAW)
            && !sourceText.equals(AlchemyAPI_KeywordParams.RAW)
            && !sourceText.equals(AlchemyAPI_KeywordParams.CQUERY)
   ...
   public String getParameterString() {
      ...
      try {
         ...
         if (keywordExtractMode != null)
            retString += "&keywordExtractMode="
                  + URLEncoder.encode(keywordExtractMode, "UTF-8");

...
public class Crawler {
         ...
         try {
            ...
            switch (RType.valueOf((String) next.get("rType"))) {
               case LINKEDIN_COMPANY:
                  resources.put(new ResourceToBeAnalyzed(Converter.toObject(
               ...
               case TWITTER_PAGE:
                  ...
                  break;
               case TWEET:
                  resources.put(new ResourceToBeAnalyzed(Converter.toObject(Tweet.class,
                        next), next));
                  break;
               case FACEBOOK_PAGE:
                  resources.put(new ResourceToBeAnalyzed(Converter.toObject(
   ...
   public static void main(String[] args) throws InterruptedException {
      ...
      List<String> subList = pages.subList(1500, 2000);
      BasicDBObject query = new BasicDBObject("from._id", new BasicDBObject("$in", subList));
      DBCursor cursor = fColl.find(query).batchSize(10);
      System.out.println(cursor.size());
      Crawler crawler = new Crawler();
      crawler.start(cursor);
      // while (cursor.hasNext()) {
